import { AcaDefect, AcaFuelLow, AcaHeadingToBase, Message, MissileToOwnshipDetected, RequestApprovalToAttack, SimToCmMessage } from "@message-schemas/schema-types";
...
//                1  2  fuel  3  ret  4  5  dfct  6  7
const TIMELINE = [0, 10, 12, 24, 25, 28, 45, 50, 55, 60];

const PORT = 6969;
const wss = new WebSocketServer({ port: PORT });
console.log(LOG.INFO, `running on port ${PORT}`);

function delayMs(msec: number): Promise<"foo"> {
...
wss.on("connection", async (ws) => {
  ...
  for (const idx in TIMELINE) {
    const delay = TIMELINE[idx] - last;
    await delayMs(delay * 1000);
    const msg: SimToCmMessage = {message: SAMPLES[idx]};
